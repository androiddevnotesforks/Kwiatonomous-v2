...
class DeviceDaoImpl(private val database: KwiatonomousDatabase) : DeviceDao {
    ...
    override fun getAllDevices(): List<Device> = transaction(database.db) {
    ...
    override fun getDevice(deviceId: String): Device? = transaction(database.db) {
    ...
    override fun createDevice(deviceId: String, birthday: Long?): Unit = transaction(database.db) {
        Devices.insert {
    ...
    override fun updateDevice(deviceId: String, lastUpdate: Long): Unit = transaction(database.db) {
        Devices.update(
            where = { Devices.deviceId eq deviceId },
            body = { it[Devices.lastUpdate] = lastUpdate }
        )
    }

    override fun updateNextWatering(deviceId: String, newNextWateringTime: Long): Unit = transaction(database.db) {
        Devices.update(
            where = { Devices.deviceId eq deviceId },
            body = { it[nextWatering] = newNextWateringTime }
        )
    }

    override fun updateLastPumpCleaning(
        deviceId: String,
        newLastPumpCleaningTime: Long
    ): Unit = transaction(database.db) {
        Devices.update(
            where = { Devices.deviceId eq deviceId },
            body = { it[lastPumpCleaning] = newLastPumpCleaningTime }
        )
    }
